...
export default function CompetitionCard({ competition, onJoin }: CompetitionCardProps) {
  ...
  return (
    <div className="bg-white overflow-hidden shadow-md rounded-xl hover:shadow-xl transition-all duration-300 border border-gray-100 hover:border-blue-200 group">
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 px-6 py-4 border-b border-gray-100">
        <div className="flex justify-between items-start mb-2">
          ...
          {competition.hasJoined && (
            <span className="px-3 py-1 text-xs font-bold rounded-full bg-green-100 text-green-700 border border-green-200 shadow-sm flex-shrink-0">
              ✓ Joined
            </span>
          )}
        </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <Sparkline data={tradingData} width={100} height={20} color="#3b82f6" fillColor="rgba(59, 130, 246, 0.15)" label="Volume" />
        </div>
      </div>
      ...
      <div className="px-6 py-5">
          ...
          <div className="w-full bg-gray-200 rounded-full h-2.5 overflow-hidden">
            <div
              className={`h-2.5 rounded-full transition-all duration-500 ${
                isFull ? 'bg-red-500' : participationPercentage > 75 ? 'bg-amber-500' : 'bg-gradient-to-r from-blue-500 to-blue-600'
              }`}
        ...
        <button
          ...
          disabled={competition.status !== 'OPEN' || competition.hasJoined || isFull}
          className={`w-full px-4 py-3 rounded-lg font-semibold transition-all duration-200 shadow-sm ${
            competition.hasJoined
